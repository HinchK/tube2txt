...
2. **Build the TUI Dashboard (requires Bun):**
...
Tube2Txt is ready for deployment on [Railway](https://railway.app).

**Configuration:**

- **Builder:** Dockerfile (auto-detected via `railway.json`).
- **Env Vars:** Set `GEMINI_API_KEY` in the Railway dashboard.
- **Port:** Uses the dynamic `PORT` assigned by Railway.

**Persistent Storage:**
Railway ephemeral disks are wiped on redeploy. For a production-like setup, mount a volume (e.g. `/app/projects`) to persist your library and the `tube2txt.db`.
...
tube2txt-hub
```
**Note:** You MUST restart the `tube2txt-hub` process after modifying the backend or building the TUI to see changes at `http://0.0.0.0:8000`.
